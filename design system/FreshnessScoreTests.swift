...
final class FreshnessScoreTests: XCTestCase {
    ...
    private func weather(temp: Double, heavyRain: Bool = false, lightRain: Bool = false) -> WeatherData {
        WeatherData(
            currentTemp: temp,
            highTemp: temp + 3,
            lowTemp: temp - 3,
            condition: heavyRain ? .heavyRain : (lightRain ? .lightRain : .partlyCloudy),
    ...
    private func activity(
        ...
        Activity(
            ...
            lat: 47.38, lon: 8.54,
            openingHours: "Daily 9:00–17:00", openingHoursDE: "Täglich 9:00–17:00",
            indoor: indoor, category: "museum",
            duration: "2 hours", price: "CHF 10", priceDE: "CHF 10",
            recurring: recurring,
            season: nil, availableMonths: availableMonths,
    ...
    func test_outdoorVenue_coldWeather_lowWeatherScore() {
        ...
        // We can't assert exact composite without knowing all weights but we can
        // verify it scores lower than an indoor venue
