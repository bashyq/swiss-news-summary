...
**Scenario 3/6 — Escape the grey (sun or snow).** 3rd grey weekend. Where's the sun? Where's the snow? Destination finder → pick city/resort → see curated highlights, things to do, travel time → for cities with full venue data (7 cities): "Plan a day here" → planner builds agenda. For all other destinations: curated highlights + Apple Maps deep links (Find playgrounds, Find restaurants) already handle the experience without a planner handoff.
...
- **City context parameter** — planner works for any of the 7 cities with venue data, not just Zürich. For destinations without venue data, the existing sunshine/snow city cards already provide curated highlights + Apple Maps deep links — no planner handoff needed.
...
The planner currently assumes Zürich. A new `planningCity` parameter allows the planner to operate against any of the 7 cities with full venue data.
...
struct PlanningCity: Codable, Equatable {
    ...
    /// Cities with full curated data (activities + restaurants).
    /// Only these cities support the full planner experience.
    static let coveredCities: [PlanningCity] = [
        .zurich,
        // ... add remaining 6 cities from existing data
    ]
...
The planner is only invoked for covered cities. For uncovered destinations, the sunshine/snow city cards already provide the right experience: curated highlights ("Things to do"), photo, 3-day forecast, travel time, Get Directions CTA, and Apple Maps deep links (Find playgrounds, Find restaurants). No planner handoff is needed or offered for these destinations.

**"Plan a day in {city}" CTA — only on covered cities:**

The CTA appears in the expanded sunshine/snow city card ONLY when `PlanningCity.isCovered(city.id)` is true. For uncovered destinations, the existing city card experience (highlights + Maps links) is the terminal screen.

```swift
// In expanded city card (existing SunshineRankingView):
if PlanningCity.isCovered(city.id) {
    Button("Plan a day in \(city.name) →") {
        todayViewModel.planningCity = PlanningCity(id: city.id, name: city.name)
        todayViewModel.selectedPlanDay = .specific(selectedDate)
        selectedTab = .today
    }
}
// If not covered: no planner CTA shown. Existing highlights + Maps links are sufficient.
```
...
// With city context (covered city only):
...
```
DiscoverView
    → SunshineRankingView (existing — relocated from Weekend tab)
        → Expanded city card (existing — photo, forecast, Things to do, Maps links)
            → [covered city only] "Plan a day in {city} →" CTA
                → navigates to Today tab with:
...
Existing sunshine ranking feature from Weekend tab, relocated. Already fully built with:
- Switzerland map with sunshine hours per city
- Filter categories (All, Sunny >6h, Partly 3-6h, etc.)
- City ranking with travel time, distance, sunshine hours
- Expandable city cards with photo, 3-day forecast per-day sunshine bars
- "Things to do" curated highlights with Apple Maps deep links
- Get Directions, Find playgrounds, Find restaurants CTAs

**What's new:** Add a "Plan a day in {city} →" CTA to expanded city cards, shown ONLY for covered cities (the 7 cities with full venue data). This is the handoff from Flow B to Flow A. For uncovered destinations, the existing card experience (highlights + Maps links) is the terminal screen — no planner CTA.

**No new CityDetailView needed.** The existing expanded city card already serves this purpose. The only addition is the conditional planner CTA.
...
| "Plan a day in {city}" from sunshine | Discover → Sunshine | `planningCity` set. Date pre-selected. Only for covered cities. | No |
| "Plan a ski day" from snow | Discover → Snow | `planningCity` set to resort city. Date pre-selected. Only for covered cities. | No |
...
Note: No `CityDetailView` needed — the existing expanded city card in `SunshineRankingView` already provides the city detail experience. Only addition is a conditional "Plan a day in {city} →" CTA for covered cities.
...
**Step 6 — City context + planner CTA on city cards**
- Add `PlanningCity` model with `coveredCities` list
...
- Add conditional "Plan a day in {city} →" CTA to expanded sunshine/snow city cards (covered cities only)
- Wire CTA: tap → set planningCity + selectedPlanDay → navigate to Today tab
- **Test:** Tap covered sunshine city → expanded card shows "Plan a day here" CTA → tap → Today tab shows "Today in Lugano" with Lugano venues. Uncovered city → no planner CTA, existing highlights + Maps links remain the terminal experience.
...
1. **News sub-view** — stays behind Plan/News toggle for now. User is tracking which news items trigger actions over the next few weeks. Data will determine whether to absorb actionable content into Discover's nudge system, keep a dedicated feed, or remove entirely. Decision deferred deliberately, not forgotten.
...
4. **Tab naming** — "Today" vs "Plan" for the first tab. "Today" implies present-day; "Plan" is more accurate since it handles any date. Consider: keep "Today" for familiarity, the date picker makes the extended scope obvious.

5. **Expanding venue coverage** — the "Plan a day here" CTA only appears for the 7 covered cities. If sunshine frequently points to destinations outside those 7, the conversion from Flow B → Flow A will be low. Monitor which sunshine destinations users expand most often. If a destination consistently gets taps but has no planner CTA, that's the signal to add venue data there.
